    ...
    <div className="Header fixed z-50 bg-[#004471] text-white py-4 w-full top-0">
      <div className="container mx-auto px-4">
        <div className="flex justify-around items-center">
          ...
          >
            <span className={`block w-6 h-0.5 bg-current transform transition duration-300 ${isOpen ? "rotate-45 translate-y-2" : "bg-white"}`}></span>
            <span className={`block w-6 h-0.5 bg-current transition duration-300 ${isOpen ? "opacity-0" : "opacity-100 bg-white"}`}></span>
            <span className={`block w-6 h-0.5 bg-current transform transition duration-300 ${isOpen ? "-rotate-45 -translate-y-1" : "bg-white"}`}></span>
          </button>
    ...
    return (
      <li key={item[0]}>
        <Link
          ...
          onClick={() => {
            setIsOpen(false); // փակել menu
            setActiveLink(item[0]); // դարձնել կարմիր
          }}
        >
          {item[1]}
        </Link>
