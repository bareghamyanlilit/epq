...
export default async function MasnPage({ params }: Params) {
  ...
  return (
    <div className="masn  mt-18 sm:mt-26">
                  ...
                  <div key={i}>
                    <div className="relative flex-col flex sm:flex-row items-center  mb-12">
                      <img
                        src={item.img}
                        alt="img"
                        className="w-[600px] h-[250px] sm:h-[500px] object-cover "
                      />
      ...
      <div className="profs">
        <h1 className="w-full py-8 text-center text-xl sm:text-3xl bg-[#004471] text-white my-5">{department.infoProfs.title}</h1>
        <div className="flex flex-wrap gap-5 sm:gap-0 justify-around my-5">
          {department.infoProfs.info.map((item,i)=> <div key={i} className="w-max flex flex-col gap-1 sm:gap-2">
